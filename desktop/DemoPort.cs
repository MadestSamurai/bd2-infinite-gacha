...
public sealed class DemoPort:IClientPort
{
    ...
    public DemoPort()
    {
        ...
        Snapshot=new(){Account=Identity.Hash("fixture-account"),Player="演示账号",ProcessId=10,ProcessStart=100,At=DateTime.UtcNow.Ticks,Instance="fixture",Sequence=1,PoolId=55,InventoryReady=true,InventoryCount=10,ResultReady=true,Pools=[new(){Id=55,DrawId=550,Size=10,Name="无限抽抽乐 · 演示卡池",Key=Identity.Hash("fixture-pool"),Costumes=costumes}],Result=Enumerable.Range(1,10).ToArray(),Message="结果已就绪",ResultKey="initial"};
    }
