...
internal static class LocalizationTests
{
 public static string[] Sources(string ignored)
 {
  ...
  {if(file.EndsWith("DemoPort.cs"))continue;foreach(var node in CSharpSyntaxTree.ParseText(File.ReadAllText(file)).GetRoot().DescendantNodes())
   {if(node.Ancestors().OfType<MethodDeclarationSyntax>().Any(m=>m.Identifier.Text is "SmokeAsync" or "SnapshotSmoke"))continue;
