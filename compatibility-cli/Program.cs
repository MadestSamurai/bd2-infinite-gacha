...
if(args.Length==3&&args[0]=="check"){var p=HookCompiler.Prepare(args[1]);Directory.CreateDirectory(args[2]);File.WriteAllBytes(Path.Combine(args[2],"BD2InfiniteGacha.Runtime5.dll"),p.Payload);File.WriteAllText(Path.Combine(args[2],"compatibility.json"),JsonSerializer.Serialize(p.Report));Console.WriteLine(JsonSerializer.Serialize(p.Report));return;}
