...
foreach($flavor in @('Portable','Lite')){
    ...
    & dotnet publish (Join-Path $PSScriptRoot 'desktop/BD2InfiniteGacha.Desktop.csproj') -c Release --self-contained $selfContained "-p:SelfContained=$selfContained" "-p:PublishSelfContained=$selfContained" --artifacts-path $buildArtifacts --output $publish "-p:Version=$Version" "-p:EnableCompressionInSingleFile=$selfContained" -p:DebugType=None -p:DebugSymbols=false @restoreArgs --nologo
    if($LASTEXITCODE -ne 0){throw "Publish failed: $flavor"}
    $exe=Join-Path $publish 'BD2InfiniteGacha.exe'
    ...
    $configs=@(Get-ChildItem -LiteralPath (Join-Path $buildArtifacts 'bin') -Recurse -File -Filter 'BD2InfiniteGacha.runtimeconfig.json')
    ...
    $name="BD2InfiniteGacha-$Version-$flavor-win-x64"
